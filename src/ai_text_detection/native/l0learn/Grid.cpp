...
void Grid<T>::Fit() {
    ...
    if (PG.P.Specs.L0) {
        // Grid1D (unlike Grid2D) never builds P.Xy, which the classification
        // CD core dereferences. It must be y .* X on the NORMALIZED design
        // the fit actually runs on — raw-scale Xy diverges (Lipschitz steps
        // are sized for scaled columns). Scoped here; the L0 path never
        // deletes Xy, so a stack lifetime is safe.
        std::unique_ptr<T> Xy;
        if (PG.P.Specs.Classification) {
            Xy.reset(new T(matrix_vector_schur_product(Xscaled, &yscaled)));
            PG.P.Xy = Xy.get();
        }
