"""FLEET R — reduced char pack (no trigrams, Donk's steer): cv + initial +
bwt + adjacent-octgram recurrence. Solo + increment over 226."""
import numpy as np
import pandas as pd
...
from ai_text_detection.chargrams import CV_FEATURE_NAMES, chargram_features
...
OUT = "docs/exp/fleet_oct.md"
FEATS = list(CV_FEATURE_NAMES) + list(BWT_FEATURE_NAMES) + list(OCT_FEATURE_NAMES)
...
def main() -> None:
    ...
    for b in "ABC":
        ...
        for t in buckets[b].generation:
            ...
            oc = oct_repeat_features(text)
            rows.append([cg[k] for k in CV_FEATURE_NAMES]
                        + [bw[k] for k in BWT_FEATURE_NAMES]
                        + [oc[k] for k in OCT_FEATURE_NAMES])
        vals[b] = np.array(rows)
        print(f"{b} done", flush=True)

    lines = ["# FLEET R — reduced pack (cv + initial + bwt + oct)\n\n",
    ...
    for arm, get in (("panel226", lambda b: prep(Xp[b])),
                     ("panel+238", lambda b: np.column_stack([prep(Xp[b]), prep_new(b)])),
                     ("pack12 alone", lambda b: prep_new(b))):
