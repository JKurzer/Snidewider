"""FLEET J — function-word pack: solo bench + increment over the 153 panel.

Usage: .venv\\Scripts\\python scripts\\exp\\fleet_funcwords.py
"""
...
from ai_text_detection.funcwords import FUNCWORD_FEATURE_NAMES, funcword_features
...
OUT = "docs/exp/fleet_funcwords.md"
...
def main() -> None:
    ...
    vals = {}
    for b in "ABC":
        rows = [[funcword_features(str(t))[k] for k in FUNCWORD_FEATURE_NAMES]
                for t in buckets[b].generation]
        vals[b] = np.array(rows)
        print(f"{b} done", flush=True)

    lines = ["# FLEET J — function-word pack\n\n",
             "| feature | AUROC B | AUROC C | TPR@1e-2 B | TPR@1e-2 C |\n|---|---|---|---|---|\n"]
    for j, f in enumerate(FUNCWORD_FEATURE_NAMES):
    ...
    for arm, get in (("panel153", lambda b: prep(Xp[b])),
                     ("panel+fw166", lambda b: np.column_stack([prep(Xp[b]), prep_new(b)])),
                     ("fw13 alone", lambda b: prep_new(b))):
