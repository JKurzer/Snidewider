"""FLEET Q — char-trigrams + CV skeleton + BWT/Wheeler run stats (sdsl-powered)."""
import numpy as np
import pandas as pd
...
OUT = "docs/exp/fleet_chargrams.md"
FEATS = list(CHARGRAM_FEATURE_NAMES) + list(BWT_FEATURE_NAMES)
...
def main() -> None:
    ...
    lines = ["# FLEET Q — chargrams + BWT/Wheeler stats; top 20 by B AUROC\n\n",
    ...
    for arm, get in (("panel226", lambda b: prep(Xp[b])),
                     (f"panel+{226 + n_new}", lambda b: np.column_stack([prep(Xp[b]), prep_new(b)])),
                     (f"pack{n_new} alone", lambda b: prep_new(b))):
