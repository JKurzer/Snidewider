"""FLEET Y — the anchor pack (self-anchors, colinear chain, anchored-context
CK2): solo (both tails) + increment over the 253 panel + timing."""
import time
...
def main() -> None:
    ...
    for b in "ABC":
        ...
        for t in buckets[b].generation:
            rows.append([anchor_features(str(t))[k] for k in ANCHOR_FEATURE_NAMES])
        vals[b] = np.array(rows)
        ms = (time.perf_counter() - t0) / len(rows) * 1000
        print(f"{b} done ({ms:.1f} ms/doc)", flush=True)

    lines = ["# FLEET Y — anchor pack (solo)\n\n",
             "| feature | AUROC B | AUROC C | TPR@1e-2 B | TPR@1e-2 C | TPR@1e-3 B | TPR@1e-3 C |\n|---|---|---|---|---|---|---|\n"]
    for j, f in enumerate(ANCHOR_FEATURE_NAMES):
    ...
    p_means = means
    a_means = np.nan_to_num(np.nanmean(vals["A"], axis=0))

    lines.append("\n## HGB increment (train A, read C)\n\n")
    lines.append("| arm | n | AUROC C | TPR@1e-2 C | TPR@1e-3 C |\n|---|---|---|---|---|\n")
    for arm, get in ((f"panel{n_panel}", lambda b: prep(Xp[b], p_means)),
                     (f"panel+{n_panel + 8}",
                      lambda b: np.column_stack([prep(Xp[b], p_means),
                                                 prep(vals[b], a_means)])),
                     ("anchor8 alone", lambda b: prep(vals[b], a_means))):
